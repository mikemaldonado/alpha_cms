...
gulp.task('sass', function(){
  ...
  return gulp.src(SOURCEPATHS.sassApp)
      .pipe(sass({outputStyle: 'nested', precision: 3,
    	includePaths: [
    		BOWERPATH.bowerDir + '/bootstrap-sass-official/assets/stylesheets',
    		BOWERPATH.bowerDir + '/bootswatch-sass',
...
gulp.task('moveFonts', ['clean-fonts'], function() {
  return gulp.src( [BOWERPATH.bowerDir + '/font-awesome/fonts/*.{eot,svg,ttf,woff,woff2}',
	  BOWERPATH.bowerDir + '/bootstrap-sass-official/assets/fonts/bootstrap/*.{eot,svg,ttf,woff,woff2}'] )
      .pipe(gulp.dest(APPPATH.fonts));
  //move fonts to single file
  //Note: must change icon-font-path for glyphicons since default path is bootstrap/fonts
});

gulp.task('scripts', ['clean-scripts'],  function() {
	return  gulp.src([BOWERPATH.bowerDir + '/jquery/dist/jquery.js', BOWERPATH.bowerDir + '/bootstrap-sass-official/assets/javascripts/bootstrap.js', SOURCEPATHS.jsSource])
      .pipe(concat('main.js'))
...
gulp.task('compress',  function() {
	...
	return  gulp.src([BOWERPATH.bowerDir + '/jquery/dist/jquery.js', BOWERPATH.bowerDir + '/bootstrap-sass/assets/javascripts/bootstrap.js', SOURCEPATHS.jsSource])
    .pipe(concat('main.js'))
      .pipe(minify())
      .pipe(gulp.dest(APPPATH.js));
